        ...
        switch (storage.users.get(userId)) {
            ...
            case (?user) {
                let updatedUser: UserTypes.User = {
                    user with
                    name = switch (update.name) { case (?name) name; case null user.name };
                    phone = switch (update.phone) { case (?phone) ?phone; case null user.phone };
                    address = switch (update.address) { case (?address) ?address; case null user.address };
                    updatedAt = Time.now();
                };
